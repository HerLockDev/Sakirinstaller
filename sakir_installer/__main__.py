...
async def oturumacvebotlogolustur (stri, aid, ahash):
    try:
        ...
        Photo = await Client.upload_file(file='sakıruserbot.jpg')
...
if __name__ == "__main__":
    ...
    if os.path.isdir("./SakirUserBot/"):
        rm_r("./EmiliaUserBot/")
    repo = Repo.clone_from(str1,"./EmiliaUserBot/", branch="master")
    ...
    config['DEFAULT_BIO'] = "✨ @EmiliaUserBot"
    ...
    config['TZ'] = TZ
    config['TZ_NUMBER'] = "1"
    config['UPSTREAM_REPO_URL'] = "https://github.com/SakirBey1/SakirUserBot"
